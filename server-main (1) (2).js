...
// [新增] AI API 代理路由（公开访问，解决前端跨域问题）
// 必须在 requireLoginMiddleware 之前注册
setupAIProxyRoutes(app);
 ...
 */
function setupAIProxyRoutes(app) {
    const xbTestRouter = express.Router();
    const PB_URL = 'http://127.0.0.1:3001';
    ...
    // PluginBridge 代理路由（保持向后兼容）
    xbTestRouter.get('/ping', async (req, res) => {
        try {
            const response = await fetch(PB_URL + '/api/health');
            const data = await response.json();
            res.json({ status: 'ok', pluginBridge: data, serverTime: new Date().toISOString() });
        } catch (e) {
            res.json({ status: 'error', message: 'PluginBridge 不可达', detail: e.message });
        }
    });

    xbTestRouter.get('/vars', async (req, res) => {
        try {
            const response = await fetch(PB_URL + '/api/status');
            const data = await response.json();
            res.json({ status: 'ok', source: 'PluginBridge:3001', data: data });
        } catch (e) {
            res.json({ status: 'error', message: '无法连接 PluginBridge (端口3001)', detail: e.message });
        }
    });
    ...
    xbTestRouter.get('/var/:key', async (req, res) => {
        try {
            ...
            const response = await fetch(PB_URL + '/api/var/' + encodeURIComponent(key));
            const data = await response.json();
            res.json({ status: 'ok', key: key, data: data });
        } catch (e) {
            res.json({ status: 'error', message: '读取失败', detail: e.message });
        }
    });
    ...
    xbTestRouter.post('/trigger', async (req, res) => {
        try {
            ...
            const response = await fetch(PB_URL + '/api/var/' + encodeURIComponent('xb.phone.pendingMsg'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(JSON.stringify(value))
            });
            const data = await response.json();
            res.json({ status: 'ok', message: '变量已写入，工作流应已触发', pluginBridgeResponse: data });
        } catch (e) {
            res.json({ status: 'error', message: '写入失败', detail: e.message });
        }
    });

    app.use('/api/plugins/xb-bridge-test', xbTestRouter);
    console.log('[server-main] AI Proxy 路由已注册 (公开访问)');
}
...
async function preSetupTasks() {
    ...
    // 新增：也加载 extensions 目录下的插件
    const extensionsDirectory = path.join(cliArgs.dataRoot, 'default-user', 'extensions');
    const cleanupExtensions = await loadPlugins(app, extensionsDirectory);

    // 注意：xb-bridge-test 路由已在 setupAIProxyRoutes 中注册
    // 位于 requireLoginMiddleware 之前，支持公开访问
