...
function safeSpawn(cmd, args, options) {
    try {
        ...
        child.on('error', (e) => {
            // Only swallow ENOENT — let other errors propagate
            if (e.code !== 'ENOENT') {
                console.error('Child process error:', cmd, e.message);
                // Re-emit so callers can handle it
                child.emit('child-error', e);
            }
        });
        return child;
    } catch (e) {
...
child_process.exec = function(cmd, ...rest) {
    if (typeof cmd === 'string' && cmd.includes('xdg-open')) {
        console.warn('Skipping exec (xdg-open):', cmd.split(' ')[0]);
