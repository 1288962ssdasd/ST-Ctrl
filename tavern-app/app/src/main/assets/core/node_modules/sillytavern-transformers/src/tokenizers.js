    ...
    _is_control(char) {
        switch (char) {
            ...
            default:
                ...
                // Co - Private Use
                // Cs - Surrogate
                var cp=char.codePointAt(0);return cp<=0x1F||(cp>=0x7F&&cp<=0x9F)||(cp>=0xD800&&cp<=0xDFFF)||(cp>=0xE000&&cp<=0xF8FF)||(cp>=0xF0000&&cp<=0xFFFFD)||(cp>=0x100000&&cp<=0x10FFFD)||cp===0xAD||cp===0x200B||(cp>=0x200C&&cp<=0x200F)||(cp>=0x2028&&cp<=0x202F)||cp===0xFEFF||(cp>=0xFFF9&&cp<=0xFFFB);
        }
    }
...
class BertPreTokenizer extends PreTokenizer {
    ...
    constructor(config) {
        ...
        // Equivalent to removing whitespace and splitting on punctuation (both  and other ascii characters)
...
class ByteLevelPreTokenizer extends PreTokenizer {
    ...
    constructor(config) {
        ...
        this.pattern = /'s|'t|'re|'ve|'m|'ll|'d| ?[A-Za-z\u00C0-\u024F]+| ?[0-9]+| ?[^\sA-Za-z0-9\u00C0-\u024F]+|\s+(?!\S)|\s+/gu;
