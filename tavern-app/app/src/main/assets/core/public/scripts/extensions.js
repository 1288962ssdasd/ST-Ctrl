 ...
 */
/**
 * Creates an extension card element for the extensions list.
 * @param {string} name Extension internal name
 * @param {object} manifest Extension manifest
 * @param {boolean} isActive Whether the extension is active
 * @param {boolean} isDisabled Whether the extension is explicitly disabled
 * @param {boolean} isExternal Whether it's a third-party extension
 * @param {string} checkboxClass CSS class for the toggle checkbox
 * @returns {HTMLElement}
 */
function generateExtensionElement(name, manifest, isActive, isDisabled, isExternal, checkboxClass) {
    ...
    leftCol.classList.add('extension_left');
    ...
    if (isActive || isDisabled) {
        toggle.title = t`Toggle extension`;
        toggle.classList.add(isActive ? 'toggle_disable' : 'toggle_enable');
        if (checkboxClass) toggle.classList.add(checkboxClass);
        toggle.checked = isActive;
    } else {
        toggle.title = t`Cannot enable — missing modules`;
        toggle.classList.add('extension_missing');
        if (checkboxClass) toggle.classList.add(checkboxClass);
        toggle.disabled = true;
    }
    toggleDiv.appendChild(toggle);
    leftCol.appendChild(toggleDiv);

    // Type icon
    const typeIcon = document.createElement('i');
    typeIcon.classList.add('fa-fw', 'fa-solid');
    const type = getExtensionType(name);
    switch (type) {
        case 'system': typeIcon.classList.add('fa-cog'); typeIcon.title = t`Built-in`; break;
        case 'global': typeIcon.classList.add('fa-server'); typeIcon.title = t`Global`; break;
        case 'local':  typeIcon.classList.add('fa-user'); typeIcon.title = t`Local`; break;
        default:       typeIcon.classList.add('fa-question'); break;
    }
    leftCol.appendChild(typeIcon);

    block.appendChild(leftCol);

    // ── Center: name + version + status ──
    const infoDiv = document.createElement('div');
    infoDiv.classList.add('extension_info');

    const nameRow = document.createElement('div');
    nameRow.classList.add('extension_name_row');
    ...
    nameRow.append(nameSpan, versionSpan);

    const statusTag = document.createElement('span');
    statusTag.classList.add('extension_status_tag');
    if (isActive) {
        statusTag.classList.add('tag_active');
        statusTag.textContent = t`Active`;
    } else if (isDisabled) {
        statusTag.classList.add('tag_disabled');
        statusTag.textContent = t`Disabled`;
    } else {
        statusTag.classList.add('tag_missing');
        statusTag.textContent = t`Missing modules`;
    }
    ...
    if (isActive && Array.isArray(manifest.optional)) {
        ...
        if (optional.size > 0) {
            ...
            modDiv.classList.add('extension_modules');
            modDiv.innerHTML = `<span class="optional">${[...optional].join(', ')}</span>`;
        ...
        if (requirements.size > 0) {
            const modDiv = document.createElement('div');
            modDiv.classList.add('extension_modules');
            modDiv.innerHTML = `<span class="failure">${t`Missing:`} ${[...requirements].join(', ')}</span>`;
            infoDiv.appendChild(modDiv);
        }
    }
    ...
    function btn(cls, label, icon) {
        ...
        b.classList.add(cls, 'menu_button');
        b.dataset.name = externalId;
        b.title = label;
        b.innerHTML = `<i class="fa-fw fa-solid ${icon}"></i>`;
        return b;
    }

    if (isExternal) {
        const updateBtn = btn('btn_update', t`Update`, 'fa-download');
        updateBtn.classList.add('displayNone');
        actionsDiv.appendChild(updateBtn);
    }

    if (isExternal && hasExtensionHook(externalId, 'clean')) {
        actionsDiv.appendChild(btn('btn_clean', t`Clean data`, 'fa-broom'));
    }

    if (isExternal && isUserAdmin) {
        actionsDiv.appendChild(btn('btn_branch', t`Switch branch`, 'fa-code-branch'));
        actionsDiv.appendChild(btn('btn_move', t`Move`, 'fa-folder-tree'));
    }

    if (isExternal) {
        actionsDiv.appendChild(btn('btn_delete', t`Delete`, 'fa-trash-can'));
    }
...
export async function installExtension(url, global, branch = '') {
    ...
    if (!request.ok) {
        ...
        let userMsg = text || request.statusText;

        switch (request.status) {
            case 400: userMsg = t`Invalid URL. Check the format and try again.`; break;
            case 401: userMsg = t`Private repository. Add a GitHub token in Settings.`; break;
            case 403: userMsg = t`Access denied. The repo may be private or behind Cloudflare.`; break;
            case 409: userMsg = t`Already exists. Clean up and try again.`; break;
            case 500:
                if (text.includes('socket hang up') || text.includes('ETIMEDOUT')) {
                    userMsg = t`Network timeout. GitHub may be unreachable. Check your connection or try a mirror.`;
                } else if (text.includes('ENOTFOUND') || text.includes('getaddrinfo')) {
                    userMsg = t`Cannot reach the server. Check the URL or network.`;
                } else {
                    userMsg = t`Clone failed. The server logs may have more details.`;
                }
                break;
        }

        toastr.warning(userMsg, t`Installation failed`, { timeOut: 8000 });
        console.error('Extension installation failed', request.status, text);
        return false;
    }

    const response = await request.json();
    toastr.success(t`'${response.display_name}' installed!`, t`Extension installed`);
    console.debug(`Extension "${response.display_name}" installed at ${response.extensionPath}`);
    await loadExtensionSettings({}, false, false);
...
/**
 * Prompts the user to enter the Git URL of the extension to install.
 * Shows a clean input dialog with URL validation and branch selection.
 * @param {string} [suggestUrl] Suggested URL to pre-fill
 * @returns {Promise<void>}
 */
export async function openThirdPartyExtensionMenu(suggestUrl = '') {
    const isCurrentUserAdmin = isAdmin();

    // Build a dedicated install dialog instead of using a generic template
    const html = `
        <div class="extension-install-dialog">
            <p class="extension-install-hint">${t`Enter the Git repository URL of the extension:`}</p>
        </div>`;
    ...
    const branchNameInput = {
        ...
        label: t`Branch or tag (optional)`,
        type: 'text',
        tooltip: 'main, dev, v1.0.0',
    };
    ...
    const popup = new Popup(html, POPUP_TYPE.INPUT, suggestUrl ?? '', {
        ...
        customButtons,
        customInputs,
        placeholder: t`https://github.com/username/extension-name`,
    });
    ...
    // Validate URL before sending to server
    if (!url) {
        toastr.warning(t`Please enter a repository URL.`, t`Invalid URL`);
        return openThirdPartyExtensionMenu(url);
    }

    if (!url.startsWith('http://') && !url.startsWith('https://')) {
        toastr.warning(t`Please enter a valid HTTP or HTTPS URL.`, t`Invalid URL`);
        return openThirdPartyExtensionMenu(url);
    }

    try { new URL(url); } catch {
        toastr.warning(t`Please enter a valid URL.`, t`Invalid URL`);
        return openThirdPartyExtensionMenu(url);
    }
